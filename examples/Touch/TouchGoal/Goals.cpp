...
bool  Goal::test() {
  ...
  MA.Lcd.drawCentreString(name, TEXT_CENTER, TEXT_TOP, TEXT_FONT);
  start_time = millis();
...
void DragFromAtoBackgroundGoal::event_handler(Event& e) {
  if(E_DRAGGED == e) {
    if(A.contains(e.from) && MA.background.contains(e.to) && !A.contains(e.to) && !B.contains(e.to)) success = true;
  }
}
...
void DragFromBtoBackgroundGoal::event_handler(Event& e) {
  if(E_DRAGGED == e) {
    if(B.contains(e.from) && MA.background.contains(e.to) && !A.contains(e.to) && !B.contains(e.to)) success = true;
  }
}
...
void DragFromBackgroundtoAGoal::event_handler(Event& e) {
  ...
  if(E_RELEASE == e) {
    if(MA.background.contains(e.from) && !A.contains(e.from) && !B.contains(e.from) && A.contains(e.to)) success = can_succeed;
  }
}
...
void DragFromBackgroundtoBGoal::event_handler(Event& e) {
  if(E_DRAGGED == e) can_succeed = false;
  if(E_RELEASE == e) {
    if(MA.background.contains(e.from) && !A.contains(e.from) && !B.contains(e.from) && B.contains(e.to)) success = can_succeed;
  }
}
