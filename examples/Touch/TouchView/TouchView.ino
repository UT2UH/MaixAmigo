...
// Display all the touch event on the Maix Amigo screen.
...
//
void setup_events() {
  MA.background.delHandlers();
  uint16_t events              = (show_move)  ? E_ALL : (E_ALL - E_MOVE); // Show all events, or everything but E_MOVE? Controlled with A button.
  MA.background.longPressTime  = (long_press) ? 500   : 0;                // Detect long presses (500mS) or not? Controlled with B button.
  MA.background.repeatDelay    = (key_repeat) ? 200   : 0;                // Repeat press events every 200mS or not? Controlled with the C button.
  MA.background.addHandler(eventHandler, events);
}
