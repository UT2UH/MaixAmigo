...
void initgame() {
  ...
  // MA.Lcd.fillRect(0,h-26,w,239,GREY);

  // MA.Lcd.setTextDatum(TC_DATUM);
  // MA.Lcd.setTextColor(TFT_WHITE,GREY);
  // MA.Lcd.drawString("TFT_eSPI example", w/2, h-26 , 4);
}
...
void midline() {
  ...
  MA.Lcd.setAddrWindow(dashline_x, 0, dashline_w, h);
  
  for(int16_t i = 0; i < dashline_n; i+=2) {
    MA.Lcd.pushColor(TFT_WHITE, dashline_w*dashline_h); // push dash pixels
    MA.Lcd.pushColor(TFT_BLACK, dashline_w*dashline_h); // push gap pixels
  }
}
...
void lpaddle() {
  ...
  if (lpaddle_d == 1) {
    MA.Lcd.fillRect(lpaddle_x, lpaddle_y, paddle_w, 1, TFT_BLACK);
  } 
  else if (lpaddle_d == -1) {
    MA.Lcd.fillRect(lpaddle_x, lpaddle_y + paddle_h - 1, paddle_w, 1, TFT_BLACK);
  }
  ...
  MA.Lcd.fillRect(lpaddle_x, lpaddle_y, paddle_w, paddle_h, TFT_WHITE);
}

void rpaddle() {
  
  if (rpaddle_d == 1) {
    MA.Lcd.fillRect(rpaddle_x, rpaddle_y, paddle_w, 1, TFT_BLACK);
  } 
  else if (rpaddle_d == -1) {
    MA.Lcd.fillRect(rpaddle_x, rpaddle_y + paddle_h - 1, paddle_w, 1, TFT_BLACK);
  }
  ...
  MA.Lcd.fillRect(rpaddle_x, rpaddle_y, paddle_w, paddle_h, TFT_WHITE);
}
...
void ball() {
  ...
  //MA.Lcd.fillRect(oldball_x, oldball_y, ball_w, ball_h, TFT_BLACK);
  MA.Lcd.drawRect(oldball_x, oldball_y, ball_w, ball_h, TFT_BLACK); // Less TFT refresh aliasing than line above for large balls
  MA.Lcd.fillRect(   ball_x,    ball_y, ball_w, ball_h, TFT_WHITE);
  oldball_x = ball_x;
  oldball_y = ball_y;
}
