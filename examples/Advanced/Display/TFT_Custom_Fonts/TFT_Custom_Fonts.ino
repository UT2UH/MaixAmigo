...
void loop() {
  ...
  MA.Lcd.fillScreen(TFT_MAGENTA);            // Clear screen
  MA.Lcd.setFreeFont(FF18);                 // Select the font
  MA.Lcd.drawString("Yellowtail 32", 160, 60, GFXFF);// Print the string name of the font
  MA.Lcd.setFreeFont(CF_Y32);                 // Select the font
  MA.Lcd.drawString(TEXT, 160, 120, GFXFF);// Print the string name of the font
  delay(2000);

  MA.Lcd.fillScreen(TFT_BLUE);            // Clear screen
  MA.Lcd.setFreeFont(FF18);                 // Select the font
  MA.Lcd.drawString("Satisfy 24", 160, 60, GFXFF);// Print the string name of the font
  MA.Lcd.setFreeFont(CF_S24);                 // Select the font
  MA.Lcd.drawString(TEXT, 160, 120, GFXFF);// Print the test text in the custom font
  delay(2000);

  MA.Lcd.fillScreen(TFT_RED);            // Clear screen
  MA.Lcd.setFreeFont(FF18);                 // Select the font
  MA.Lcd.drawString("Roboto 24", 160, 60, GFXFF);// Print the string name of the font
  MA.Lcd.setFreeFont(CF_RT24);                 // Select the font
  MA.Lcd.drawString(TEXT, 160, 120, GFXFF);// Print the test text in the custom font
  delay(2000);

  MA.Lcd.fillScreen(TFT_DARKGREY);            // Clear screen
  MA.Lcd.setFreeFont(FF18);                 // Select the font
  MA.Lcd.drawString("Orbitron 24", 160, 60, GFXFF);// Print the string name of the font
  MA.Lcd.setFreeFont(CF_OL24);                 // Select the font
  MA.Lcd.drawString(TEXT, 160, 120, GFXFF);// Print the test text in the custom font
  delay(2000);
  
  // Here we do not clear the screen and rely on the new text over-writing the old
  MA.Lcd.setFreeFont(FF18);                 // Select the font
  MA.Lcd.drawString("Orbitron 32", 160, 60, GFXFF);// Print the string name of the font
  MA.Lcd.setFreeFont(CF_OL32);                 // Select the font
  MA.Lcd.drawString(TEXT, 160, 120, GFXFF);// Print the test text in the custom font
  delay(2000);

  // Here we use text background padding to over-write the old text
  MA.Lcd.fillScreen(TFT_YELLOW);            // Clear screen
  MA.Lcd.setTextColor(TFT_WHITE, TFT_BLACK);

  // Here we use text background padding to over-write the old text
  MA.Lcd.setTextPadding(MA.Lcd.width() - 20); // Blanked area will be width of screen minus 20 pixels
  MA.Lcd.setFreeFont(FF18);                 // Select the font
  MA.Lcd.drawString("Orbitron 32 with padding", 160, 60, GFXFF);// Print the string name of the font
  MA.Lcd.setFreeFont(CF_OL32);                 // Select the font
  MA.Lcd.drawString(TEXT, 160, 120, GFXFF);// Print the test text in the custom font
  delay(2000);
  ...
  MA.Lcd.setTextPadding(80);
  MA.Lcd.setTextDatum(MC_DATUM);
  MA.Lcd.setFreeFont(CF_OL32);
  for( int i = 100; i > 0; i--)
  {
    MA.Lcd.drawNumber( i, 160, 200);
    delay(500);
  }

  // Reset text padding to zero (default)
  MA.Lcd.setTextPadding(0);
}
