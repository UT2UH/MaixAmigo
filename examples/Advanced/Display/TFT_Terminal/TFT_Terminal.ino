...
void setup() {
  ...
  MA.begin();
  // MA.Lcd.setRotation(5); // Must be setRotation(0) for this sketch to work correctly
  ...
  MA.Lcd.fillRect(0,0,320,TEXT_HEIGHT, TFT_BLUE);
  MA.Lcd.drawCentreString(" Serial Terminal - 115200 baud ",320/2,0,2);
...
void loop(void) {
  ...
  //  if (change_colour){
  //  change_colour = 0;
  //  if (selected == 1) {MA.Lcd.setTextColor(TFT_CYAN, TFT_BLACK); selected = 0;}
  //  else {MA.Lcd.setTextColor(TFT_MAGENTA, TFT_BLACK); selected = 1;}
  //}
  ...
  while (Serial.available()) {
    ...
    if (data > 31 && data < 128) {
      xPos += MA.Lcd.drawChar(data,xPos,yDraw,2);
...
int scroll_line() {
  ...
  // MA.Lcd.fillRect(0,yStart,blank[(yStart-TOP_FIXED_AREA)/TEXT_HEIGHT],TEXT_HEIGHT, TFT_BLACK);
  MA.Lcd.fillRect(0,yStart,320,TEXT_HEIGHT, TFT_BLACK);
...
void setupScrollArea(uint16_t tfa, uint16_t bfa) {
  MA.Lcd.writecommand(ILI9341_VSCRDEF); // Vertical scroll definition
  MA.Lcd.writedata(tfa >> 8);           // Top Fixed Area line count
  MA.Lcd.writedata(tfa);
  MA.Lcd.writedata((YMAX-tfa-bfa)>>8);  // Vertical Scrolling Area line count
  MA.Lcd.writedata(YMAX-tfa-bfa);
  MA.Lcd.writedata(bfa >> 8);           // Bottom Fixed Area line count
  MA.Lcd.writedata(bfa);
}
...
void scrollAddress(uint16_t vsp) {
  MA.Lcd.writecommand(ILI9341_VSCRSADD); // Vertical scrolling pointer
  MA.Lcd.writedata(vsp>>8);
  MA.Lcd.writedata(vsp);
}
