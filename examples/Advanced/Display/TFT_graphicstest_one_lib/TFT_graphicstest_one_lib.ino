...
unsigned long testText() {
  ...
  MA.Lcd.setTextColor(TFT_WHITE);  MA.Lcd.setTextSize(1);
  MA.Lcd.println("Hello World!");
  MA.Lcd.setTextColor(TFT_YELLOW); MA.Lcd.setTextSize(2);
  MA.Lcd.println(1234.56);
  MA.Lcd.setTextColor(TFT_RED);    MA.Lcd.setTextSize(3);
  MA.Lcd.println(0xDEADBEEF, HEX);
  MA.Lcd.println();
  MA.Lcd.setTextColor(TFT_GREEN);
  MA.Lcd.setTextSize(5);
  ...
  MA.Lcd.println("Or I will rend thee");
  MA.Lcd.println("in the gobberwarts");
  MA.Lcd.println("with my blurglecruncheon,");
  MA.Lcd.println("see if I don't!");
  return micros() - start;
}
...
unsigned long testLines(uint16_t color) {
  ...
  y2    = h - 1;
  start = micros();
  for (x2 = 0; x2 < w; x2 += 6) MA.Lcd.drawLine(x1, y1, x2, y2, color);
  x2    = w - 1;
  for (y2 = 0; y2 < h; y2 += 6) MA.Lcd.drawLine(x1, y1, x2, y2, color);
...
unsigned long testFastLines(uint16_t color1, uint16_t color2) {
  ...
  MA.Lcd.fillScreen(TFT_BLACK);
  start = micros();
  for (y = 0; y < h; y += 5) MA.Lcd.drawFastHLine(0, y, w, color1);
  for (x = 0; x < w; x += 5) MA.Lcd.drawFastVLine(x, 0, h, color2);
