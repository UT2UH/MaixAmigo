...
TFT_eSprite spr = TFT_eSprite(&MA.Lcd);
...
void loop(void)
{
  ...
  spr.pushSprite(MA.Lcd.width() / 2 - WIDTH / 2, MA.Lcd.height() / 2 - HEIGHT / 2);
  spr.pushSprite(MA.Lcd.width() - WIDTH + 40, MA.Lcd.height() - HEIGHT + 40);
  ...
  int x = MA.Lcd.width() / 2  -  WIDTH / 2;
  int y = MA.Lcd.height() / 2 - HEIGHT / 2;
  ...
  while (true)
  {
    ...
    if (x < -WIDTH/2) dx = 1;
    if (x >= MA.Lcd.width()-WIDTH/2) dx = -1;
    if (y < -HEIGHT/2) dy = 1;
    if (y >= MA.Lcd.height()-HEIGHT/2) dy = -1;
