...
void setup() {
  ...
  MA.Lcd.fillScreen(ILI9341_TFT_BLACK);
...
void loop(void) {
  ...
  for (int j = 0; j < 600; j += TEXT_HEIGHT) {
    for (int i = 0; i < 40; i++) {
      ...
      MA.Lcd.setTextColor(pos[i] << 5, ILI9341_TFT_BLACK); // Set the green character brightness
      if (pos[i] == 63) MA.Lcd.setTextColor(ILI9341_TFT_WHITE, ILI9341_TFT_BLACK); // Draw white character
      xPos += MA.Lcd.drawChar(random(32, 128), xPos, yDraw, 1); // Draw the character
    }
  ...
  //MA.Lcd.setRotation(2);
  //MA.Lcd.setTextColor(63 << 5, ILI9341_TFT_BLACK);
  //MA.Lcd.drawCentreString("MATRIX",120,60,4);
...
void setupScrollArea(uint16_t TFA, uint16_t BFA) {
  ...
  MA.Lcd.writedata(TFA >> 8);
  MA.Lcd.writedata(TFA);
  MA.Lcd.writedata((320 - TFA - BFA) >> 8);
  MA.Lcd.writedata(320 - TFA - BFA);
  MA.Lcd.writedata(BFA >> 8);
...
void scrollAddress(uint16_t VSP) {
  MA.Lcd.writecommand(ILI9341_VSCRSADD); // Vertical scrolling start address
