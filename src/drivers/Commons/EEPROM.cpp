...
#define DEFAULT_ADDRESS 0x00006000 // based on Kboot reserved area
#define SPI_FLASH_SEC_SIZE w25qxx_FLASH_SECTOR_SIZE
...
: _sector((DEFAULT_ADDRESS / SPI_FLASH_SEC_SIZE)) // reserved area
, _data(0)
, _size(0)
, _dirty(false)
{
...
void EEPROMClass::begin(size_t size) {
  ...
  Maix.flashInit();
  if (!Maix.flashRead(_sector * SPI_FLASH_SEC_SIZE, reinterpret_cast<uint32_t*>(_data), _size)) {
...
bool EEPROMClass::commit() {
  ...
  if (Maix.flashEraseSector(_sector)) {
    if (Maix.flashWrite(_sector * SPI_FLASH_SEC_SIZE, reinterpret_cast<uint32_t*>(_data), _size)) {
      _dirty = false;
      return true;
    }
  }
