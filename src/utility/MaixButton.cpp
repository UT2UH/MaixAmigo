...
void Button::erase(uint16_t color /* = TFT_BLACK */) {
...
/* static */ MaixButtons* MaixButtons::instance;

/* static */ void MaixButtons::drawFunction(Button& b, ButtonColors bc) {
  ...
  if (bc.text != NODRAW && bc.text != bc.bg && strlen(b._label)) {
    ...
    if (!b._compat) {
      //uint8_t margin = max(r / 2, 6);   //TODO FIX!
      uint8_t margin = ((r / 2) > 6 ? (r / 2) : 6);
      switch (b.datum) {
        case TL_DATUM:
        case ML_DATUM:
        case BL_DATUM:
...
Button* MaixButtons::which(Point& p) {
...
void MaixButtons::draw() {
  for (auto button : Button::instances) button->draw();
}

void MaixButtons::update() {
#ifdef _MAIXTOUCH_H_
...
void MaixButtons::setFont(const GFXfont* freeFont_) {
  _freeFont = freeFont_;
  _textFont = 1;
}

void MaixButtons::setFont(uint8_t textFont_) {
  _freeFont = nullptr;
  _textFont = textFont_;
}

void MaixButtons::setTextSize(uint8_t textSize_) { _textSize = textSize_; }

void MaixButtons::fireEvent(uint8_t finger, uint16_t type, Point& from,
...
void MaixButtons::addHandler(void (*fn)(Event&), uint16_t eventMask /* = E_ALL */,
...
void MaixButtons::delHandlers(void (*fn)(Event&) /* = nullptr */,
                            Button* button /* = nullptr */,
                            Gesture* gesture /* = nullptr */
) {
