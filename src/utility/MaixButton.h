...
== MaixButton: Buttons, Gestures and Events ==
  ...
  * Buttons on the screen, either as labels above the original M5Stack's
    hardware buttons or anywhere on the touch screen of the Maix Amigo.
  ...
  * Emulation of the (much less feature-rich) TFT_eSPI_Button class. This
  ...
  This library was written for the Maix series of devices, but was made to
...
== Buttons ==
  ...
  the controller. On the Maix Amigo, you can also create buttons that act
  ...
  Whether on the Maix boards with hardware buttons or on the Maix Amigo with a touch
  screen, buttons are special forms of the 'Zone' object, meaning that all
  functions that apply to 'Zone' objects also work on buttons. On the Maix device
...
== Hardware Buttons ==
  ...
  Maix device with buttons, this sketch will output a star to the serial port
  every time you release the button. (And because pin 39 is the interrupt
  wire for the touch screen on the Maix Amigo, it also happens to output a star on
  that device every time you touch the screen.)

    Note that the sketch uses 'MA.update()' instead of 'myButton.read()'. You
    don't need to read() your buttons explicitly anymore. All buttons created
    with MaixButton are automatically read by 'MA.update()'. (Unless you read
    them with 'myButton.read()', in which case 'MA.update()' stops doing that
    to avoid you missing things.)

  The next sections will describe buttons and gestures on the touch screen,
  but if you have an Maix device without a touch screen: keep reading
...
== Buttons Using the Touch Screen ==
    ...
    interface that is underneath the MaixButton library.
  ...
  On the Maix Amigo the three buttons MA.BtnA, MA.BtnB and MA.BtnC from the older
...
== Buttons with visual appearance ==
  ...
  line by:
    ...
    ButtonColors col = {TFT_BLACK, TFT_WHITE, TFT_WHITE};
  ...
  is, regardless of whether it's pressed. Thus, if we say
    ...
    ButtonColors onCol = {TFT_BLACK, TFT_WHITE, TFT_WHITE};
    ButtonColors offCol = {TFT_RED, TFT_WHITE, TFT_WHITE};
  ...
  colors can also be addressed directly. "myButton.on.bg = TFT_BLUE;" will turn
  ...
  buttons do that with "MA.Buttons.draw()". You can also call draw with a
  ButtonColors variable so "myButton.draw({TFT_BLUE, TFT_WHITE, TFT_WHITE})" draws it
...
== Visual Buttons (Labels) with Hardware Buttons ==
  ...
  on the Maix device:
    ...
    void setup() {
      MA.BtnA.off = MA.BtnB.off = MA.BtnC.off = {TFT_BLUE, TFT_WHITE, NODRAW};
      MA.BtnA.on  = MA.BtnB.on  = MA.BtnC.on  = {TFT_RED,  TFT_WHITE, NODRAW};
      MA.begin();
      MA.Buttons.draw();
    }
...
== MA.Buttons ==
  ...
  there is an instance called "MA.Buttons" (plural), that is used to talk to
  the MaixButton library for things that involve all buttons. For instance:
  "MA.Buttons.setFont" sets a font for all buttons, and you can use
  "MA.Buttons.addHandler" to add a handler that gets events for all buttons
  (and gestures).
...
== Events ==
  ...
  Events are MaixButton's way of making it easier to react to events on
  ...
  Here's an events-based sketch for the Maix Amigo. We'll base it on the same
  ...
  function to receive events:
    ...
    MA.Buttons.addHandler(myHandler);
  ...
  E_PRESSING and E_LONGPRESSING
    ...
    MaixButton is sure that's not just a short tap (more later). The maximum
  ...
  E_TAP, E_DBLTAP, E_PRESSED, E_LONGPRESSED and E_DRAGGED
    ...
    (E_TAP takes a tiny bit longer before it fires because MaixButton needs to
  ...
  an event. That event has all sorts of properties that we can look at.
    ...
    e.finger
      ...
      touch screen. Left at zero on the Maix device with buttons.

    e.from and e.to

      Points that say from where to where this event happened. Left at
      invalid for the Maix device with buttons.
  ...
  Here are some examples of ways to add a handler function:
    ...
    MA.Buttons.addHandler(btnHandle, E_ALL - E_MOVE);
...
== Taps, Doubletaps, Longpresses and Key Repeat ==
  ...
  Some features are best explained with some examples:
    ...
    mybutton.tapWait = 0;
      ...
      wait before an E_TAP fires, because MaixButton is still waiting to see if
...
== In Loop vs. Event Handlers ==
  ...
  that was detected this time around by 'MA.update()'. Each event comes in
  it's own rotation of 'MA.update()', so if you prefer to detect events this
  ...
  of 0, so you can do "if (myButton.event) ...". 'MA.Buttons.event' has the
  ...
  attached to. This example prints a star to serial if it is doubletapped.
    ...
    Button myButton(50,70,220, 100, false, "Button",
                    {TFT_YELLOW, TFT_BLACK, NODRAW},
                    {TFT_RED, TFT_BLACK, NODRAW} );

    void setup() {
      MA.begin();
      MA.Buttons.setFont(FSS18);
      MA.Buttons.draw();
    }
...
== MA.background ==
  ...
  One special button, "MA.background", was defined before any others, and it
...
== Advanced Hints and Tricks
  ...
  ## drawFn
    ...
    drawing of the button is done by a static function in the MaixButtons
    object. It's defined as

      void MaixButtons::drawFunction(Button& b, ButtonColors bc)
    ...
    "MA.Buttons.drawFn = myFunction". You can even do that on a per-button
  ...
  ## drawZone
    ...
    buttons on the Maix Amigo on the screen just above their touch areas.


  ## Drawing is Non-Invasive

    This library uses a brand-new feature of the MaixDisplay object --
    MA.Lcd.popState() and MA.lcd.pushState() -- that allows it to save and
  ...
  ## Buttons and Variable Scope
    ...
    screen anyway, this can be done with "myButton.erase(TFT_BLACK)" if the
    background is to be black.

*/

#ifndef _MAIXBUTTON_H_
#define _MAIXBUTTON_H_
