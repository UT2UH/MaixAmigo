...
// So we can use this instance without including all of Maix Amigo 
MaixDisplay* MaixDisplay::instance;

MaixDisplay::MaixDisplay() : TFT_eSPI() {
  if (!instance) instance = this;
}

void MaixDisplay::begin() {
  TFT_eSPI::begin();
...
void MaixDisplay::sleep() {
  startWrite();
  writecommand(0x10); // // ILI9486_SLPIN = ST7789_SLPIN, Software reset?
  endWrite();
}

void MaixDisplay::wakeup() {
  startWrite();
  writecommand(0x11); // ILI9486_SLPOUT = ST7789_SLPOUT
  endWrite();
}

void MaixDisplay::setBrightness(uint8_t brightness) {
  // ledcWrite(BLK_PWM_CHANNEL, brightness);
}

void MaixDisplay::drawBitmap(int16_t x0, int16_t y0, int16_t w, int16_t h, const uint16_t *data) {
...
void MaixDisplay::drawBitmap(int16_t x0, int16_t y0, int16_t w, int16_t h, uint16_t *data) {
  bool swap = getSwapBytes();
  setSwapBytes(true);
  pushImage((int32_t)x0, (int32_t)y0, (uint32_t)w, (uint32_t)h, data);
  setSwapBytes(swap);
...
void MaixDisplay::drawBitmap(int16_t x0, int16_t y0, int16_t w, int16_t h, const uint16_t *data, uint16_t transparent) {
  bool swap = getSwapBytes();
  setSwapBytes(true);
...
void MaixDisplay::drawBitmap(int16_t x0, int16_t y0, int16_t w, int16_t h, const uint8_t *data) {
  bool swap = getSwapBytes();
  setSwapBytes(true);
...
void MaixDisplay::drawBitmap(int16_t x0, int16_t y0, int16_t w, int16_t h, uint8_t *data) {
  bool swap = getSwapBytes();
  setSwapBytes(true);
...
void MaixDisplay::progressBar(int x, int y, int w, int h, uint8_t val) {
  drawRect(x, y, w, h, 0x09F1);
  fillRect(x + 1, y + 1, w * (((float)val) / 100.0), h - 1, 0x09F1);
}

#include "helpers/qrcode.h"
void MaixDisplay::qrcode(const char *string, uint16_t x, uint16_t y, uint8_t width, uint8_t version) {
...
void MaixDisplay::qrcode(const String &string, uint16_t x, uint16_t y, uint8_t width, uint8_t version) {
...
// void MaixDisplay::drawBmpFile(fs::FS &fs, const char *path, uint16_t x, uint16_t y) {
...
// void MaixDisplay::drawBmp(fs::FS &fs, const char *path, uint16_t x, uint16_t y) {
...
// void MaixDisplay::drawJpg(const uint8_t *jpg_data, size_t jpg_len, uint16_t x,
...
// void MaixDisplay::drawJpgFile(fs::FS &fs, const char *path, uint16_t x, uint16_t y,
...
// void MaixDisplay::drawPngFile(fs::FS &fs, const char *path, uint16_t x, uint16_t y,
//                             uint16_t maxWidth, uint16_t maxHeight, uint16_t offX,
...
// void MaixDisplay::drawPngUrl(const char *url, uint16_t x, uint16_t y,
//                             uint16_t maxWidth, uint16_t maxHeight, uint16_t offX,
//                             uint16_t offY, double scale, uint8_t alphaThreshold)
// {
//   HTTPClient http;
...
uint16_t MaixDisplay::getTouchRawZ(void) {
  return (TOUCH->ispressed()) ? 1000 : 0;
}

void MaixDisplay::convertRawXY(uint16_t *x, uint16_t *y) { return; }

uint8_t MaixDisplay::getTouch(uint16_t *x, uint16_t *y,
...
void MaixDisplay::calibrateTouch(uint16_t *data, uint32_t color_fg,
                               uint32_t color_bg, uint8_t size) {
  return;
}

void MaixDisplay::setTouch(uint16_t *data) { return; }
